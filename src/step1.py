...
def main():
    ...
    for i, transect_id in enumerate(unprocessed_transects):
        ...
        success, current_doc, _ = process_transect(transect_id, current_doc, current_psx_path)
        
        if success:
            batch_mapping[current_psx_path].append(transect_id)
        
        # Force save after each transect
        if current_doc and current_psx_path:
            current_doc.save(current_psx_path)
            logging.info(f"Saved progress to {current_psx_path}")
